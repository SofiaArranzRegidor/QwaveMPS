...
def t_evol_mar_dm(L: np.ndarray, i_s0: np.ndarray, i_n0, params):
    """
    Evolve the system in the Markovian regime using density matrices.

    Parameters
    ----------
    L : np.ndarray
        Continuous-time Liouvillian generator acting on the joint
        system-and-current-bin density matrix. The one-step map is constructed
        internally as ``expm(L * delta_t)``.

    i_s0 : np.ndarray
        Initial system density-matrix tensor.

    i_n0 : list[np.ndarray] or None
        Initial input field bins in density-matrix form. If ``None``, vacuum
        bins are generated automatically.
    ...
    -------
    Bins
        Dataclass containing the system states, input and output field states,
        correlation bins, and Schmidt coefficients generated during the
        evolution.
    """
...
def t_evol_nmar_dm(L: np.ndarray, i_s0: np.ndarray, i_n0, params):
    """
    Evolve the system in the non-Markovian regime using density matrices.

    Parameters
    ----------
    L : np.ndarray
        Continuous-time Liouvillian generator acting on the feedback bin,
        system, and current input-bin density matrix. The one-step map is
        constructed internally as ``expm(L * delta_t)``.
    ...
    -------
    Bins
        Dataclass containing the system states, loop-field states, output-field
        states, input-field states, correlation bins, and Schmidt coefficients
        generated during the evolution. The returned object also stores the
        auxiliary attributes ``bin_sys_corr_list`` and
        ``bin_sys_corr_list_tau`` used by some density-matrix correlation
        workflows.
    """
    delta_t = params.delta_t
    tmax = params.tmax
